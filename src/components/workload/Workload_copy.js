...
const numberRender = ({resultSet, pivotConfig}) => (
    ...
    <Col type="flex" justify="center" align="middle">
        <h5> Weekly Value {resultSet.seriesNames ().map ((s) => (
        ...
        <Container fluid>
            ...
            {cubeWeek && cubeYear && (
                <Row>
                    ...
                    <Col style={{marginTop: "10px", textAlign: "center"}}>
                        <h5 style={{fontWeight: "bold"}}>{`Week ${cubeWeek} Year ${cubeYear}`}</h5>
                    </Col>
                    <WeeklyValue/>
                </Row>
            )}
            {cubeWeek && cubeYear && <Divider/>}
            {cubeWeek && cubeYear && (<WorkLoadCharts yearSelection={cubeYear} weekSelection={cubeWeek}/>)}
            {!cubeWeek && !cubeYear && (
                <>
                    <Divider/>
