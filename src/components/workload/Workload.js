...
const numFormatGrid = (value) => {
	...
	const totalValue = worksheets.map(item => item.value_complete).reduce((acc, item) => acc + item, 0)
	...
	const years = [
		...new Set(workDoneWeeks.map((item) => item["iso_year"].toString()))].sort().reverse();
	...
	return (
		<Container fluid>
			...
			<hr/>
			{ worksheets.length > 0 && !isLoading && <h3 style={{textAlign: 'center'}}>Weekly Value  {numFormatGrid(totalValue)}</h3> }
			{ isLoading && (
				<Loader
