...
const availableWorksheets = createSelector(
	[selectAllAvailableWorksheets, selectedInstruction],
	(worksheets, instruction) => worksheets.filter(obj => obj.order_ref === instruction.work_instruction)
);

const AvailableWorksheetsList = () => {

	const orderWorksheets = useSelector(availableWorksheets);
	const dispatch = useDispatch();

	const ColumnDefs = [
		{headerName: "Select", colId: "select", checkboxSelection: true, maxWidth: 80, headerCheckboxSelection: false,},
		...
		{headerName: "WD Date", field: "date_work_done", valueFormatter: formatDate, type: "dateColumn"},
		{headerName: "Supervisor", field: "supervisor_name"},
		{headerName: "Qty To Apply", field: "qty_complete", type: "numericColumn", valueFormatter: numFormatGrid},
		{headerName: "Value To Apply", field: "value_complete", type: "numericColumn", valueFormatter: numFormatGrid},
	];
	const defaultColDef = {
		filter: true,
		sortable: true,
